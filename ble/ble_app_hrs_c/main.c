...
#define TARGET_UUID                 0xFFF0        /**< Target device uuid that application is looking for. */
 ...
 */
static char const m_target_periph_name[] = "TM_224F";      /**< If you want to connect to a peripheral using a given advertising name, type its name here. */
...
static ble_gap_addr_t const m_target_periph_addr =
{
    ...
    .addr      = {0x18, 0x7A, 0x93, 0x71, 0x22, 0x4F}
};
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
        {
            ...
            NRF_LOG_INFO("ble_db_discovery_start");
            err_code = ble_db_discovery_start(&m_db_disc, p_ble_evt->evt.gap_evt.conn_handle);
            APP_ERROR_CHECK(err_code);

            NRF_LOG_INFO("bsp_indication_set(BSP_INDICATE_CONNECTED)");
...
static void db_discovery_init(void)
{
    ...
    NRF_LOG_INFO("db_discovery_init");
...
//    APP_ERROR_CHECK(err_code);
    ...
    {
        NRF_LOG_INFO("===== SCAN_NAME_FILTER %s", m_target_periph_name);
        err_code = nrf_ble_scan_filter_set(&m_scan,
                                           SCAN_NAME_FILTER,
...
int main(void)
{
    ...
    // Start execution.
    NRF_LOG_INFO("======= Heart Rate collector example started 2 ======");
